...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'username', 'profile_photo', 'notification_type',
                  'message', 'sender_id', 'message_id', 'read', 'created_on']
    ...
    def get_message_id(self, obj):
        try:
            ...
            User = get_user_model()
            sender = User.objects.get(username=sender_username)
            # Assuming Message model has 'sender' and 'receiver' fields
            message = Message.objects.filter(sender=sender, receiver=obj.user).latest('timestamp')
            return message.id
        except (IndexError, User.DoesNotExist, Message.DoesNotExist):
            return None
