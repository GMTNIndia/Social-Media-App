...
function SearchBar() {
  ...
  return (
    <section className="bg-gray-100 min-h-screen p-4 md:p-8 lg:p-12 mt-20">
      <form
        className="flex flex-col items-start bg-white rounded-lg shadow-sm border border-solid border-zinc-200 p-4 max-w-full md:max-w-xl lg:max-w-5xl mx-auto"
        onSubmit={(e) => e.preventDefault()}
      >
        <label htmlFor="searchInput" className="sr-only">
          What are you looking for?
        ...
        <input
          className="w-full h-12 px-4 py-2 rounded-md bg-zinc-200 text-zinc-500 text-sm"
          type="text"
          id="searchInput"
          value={searchTerm}
          onChange={handleInputChange}
