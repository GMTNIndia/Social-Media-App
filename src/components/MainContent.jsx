    ...
    <div className="w-full md:w-2/4 p-4 mx-auto">
      ...
      {posts.map(post => (
        <div key={post.id} className="bg-white shadow rounded p-4 mb-4">
          ...
          {post.image && <img src={`http://localhost:8000/${post.image}`}alt="Post" className="mt-4 rounded w-full" />}
    ...
    <div>
      {comments.map(comment => (
        <div key={comment.id} className="bg-gray-100 p-2 rounded mt-2">
          ...
          <p className="text-xs text-gray-600">{new Date(comment.created_on).toLocaleTimeString()}</p>
